...
if (isset($_POST['ids_destinataires']))
	{
	$r_sql="SELECT `groupes`.`description` FROM `groupes` WHERE `groupes`.`id`='".$_POST['ids_destinataires']."' LIMIT 1";
	$R=mysqli_query($mysqli, $r_sql);
	if (mysqli_num_rows($R)>0)
		{
		$t_ensemble=mysqli_fetch_assoc($R);
		$ensemble_destinataire=addslashes($t_ensemble['description']);
		}
	}
...
// le rédacteur est-il associé à un ou plusieurs groupes ?
$r_sql="SELECT `groupes`.`id`,`groupes`.`description`,GROUP_CONCAT(`classes`.`nom_complet`) FROM `utilisateurs`,`j_groupes_professeurs`,`groupes`,`j_groupes_classes`,`classes` WHERE (`utilisateurs`.`login`='".$_SESSION['login']."' AND `utilisateurs`.`login`=`j_groupes_professeurs`.`login` AND `j_groupes_professeurs`.`id_groupe`=`groupes`.`id` AND `j_groupes_classes`.`id_groupe`=`groupes`.`id` AND `j_groupes_classes`.`id_classe`=`classes`.`id`) GROUP BY `groupes`.`id` ORDER BY `groupes`.`description`,GROUP_CONCAT(`classes`.`nom_complet`)";
$R_groupes=mysqli_query($mysqli, $r_sql);
if (mysqli_num_rows($R_groupes)==0) 	header("Location: saisie.php");
...
<h2>Saisie d'un mot dans un carnet de liaison de GROUPE</h2>
...
<div class="saisie" style="width: 560px;">
	...
	<form method="post" name="saisie" action="saisie_par_groupe.php" enctype="multipart/form-data">
	...
	while($un_groupe=mysqli_fetch_assoc($R_groupes)) 
		{
	?>
		<option value="<?php echo $un_groupe['id']; ?>" <?php if ((isset($_POST['ids_destinataires'])) && ($un_groupe['id']==((count($_POST['ids_destinataires'])>0)?$_POST['ids_destinataires'][0]:0))) echo "selected==\"selected\""; ?>><?php echo $un_groupe['description']." ".$un_groupe['GROUP_CONCAT(`classes`.`nom_complet`)']; ?></option>
	<?php
		}
	?>
	</select>
